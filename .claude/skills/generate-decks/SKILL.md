...
기본 경로는 이 Skill이다. `pnpm ai:generate-decks`는 Anthropic API 키가 필요한 legacy/운영자용 스크립트이므로 사용자가 명시하지 않으면 실행하지 않는다. 업로드는 초안 검수 후 `/upload-decks <decks-artifact>`를 사용한다.
...
Next: review words/tags/name/description, set "status": "approved" on keepers, then run /upload-decks <path>.
```
