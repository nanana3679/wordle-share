...
function buildSystemPrompt(): string {
  return [
    ...
    "- Between 12 and 20 distinct words using only a-z letters (no digits, spaces, apostrophes, or hyphens).",
    "- SEMANTIC COMPLETENESS > length. If a word is iconic to the topic and a reasonable fan/user would wonder 'why isn't this in the deck?', INCLUDE IT regardless of length. Examples: 'rm' (BTS member), 'iu' (singer), 'ai' (concept), 'psy' (artist), or very long names like 'seventeen', 'treasure'. Never drop an essential word just because it's 2 letters or 10+ letters.",
    "- After covering all the iconic/essential words, fill remaining slots with related words to hit the 12-20 target. Those filler words can be any length.",
    "- English words AND romanized proper nouns from any language are both great (e.g. 'minji', 'tokyo', 'hatsune', 'kimchi', 'oppa', 'bulgogi'). For K-pop / J-pop / K-drama / anime / Korean food / Japanese culture topics, lean into romanization so the deck actually reflects the topic.",
    "- No duplicates (case-insensitive).",
    "- Each word must have a clear semantic link to the topic.",
    "- Well-known names and terms are preferred over obscure jargon, but famous romanized proper nouns (idol names, character names, place names, food names) count as recognizable.",
    "",
