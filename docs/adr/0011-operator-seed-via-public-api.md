...
- 인증: `bot_*` nick + 운영자 보관 pw (예: `bot_pokemon_kr`, `bot_onepiece_kr`) + `x-bot-token` 헤더 (구현 노트 참고)
...
- ADR은 "nick + pw"만 명시했으나, 구현에서 **`bot_` prefix nick 자체를 게이트**로 추가:
  - `POST /api/decks`는 `bot_` prefix nick 요청을 **`x-bot-token` 헤더가 `BOT_SEED_TOKEN` env와 일치할 때만** 허용. 불일치 시 400 거절 (`app/api/decks/route.ts`, `lib/identity.ts`의 `isBotNick()`)
  - 일반 사용자가 `bot_` nick을 도용해 봇 사칭 덱을 만드는 것을 차단 (nick은 전역 유일이 아니므로 prefix 보호 필요)
- 시드 스크립트(`scripts/ai/generate-decks.ts`)는 `x-bot-token: BOT_SEED_TOKEN` 헤더 + body의 `nick=BOT_NICK` / `password=BOT_PW`로 호출
- 관련 env: `BOT_SEED_TOKEN`(API 게이트), `BOT_NICK` / `BOT_PW`(자격증명), `API_BASE_URL`(호출 대상). `.env.example` 참고
