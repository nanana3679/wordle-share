...
- **anon_id = `auth.uid()`** (Supabase Anonymous Auth로 발급되는 UUID). 첫 쓰기/플레이 시점에 lazy 발급 (구현 노트 참고)
...
- **lazy 발급으로 확정** (`lib/anon-session.ts`의 `getOrCreateAnonUserId()`): 미들웨어는 기존 세션 refresh만 하고(`getUser()`), 익명 세션을 **자동 발급하지 않음**
- 익명 세션은 **첫 쓰기/플레이 server action** (daily/challenge/comment/deck/report/like) 진입 시점에 `signInAnonymously()`로 발급
- 근거: 미들웨어 자동 발급이면 단순 조회·봇 방문마다 익명 유저 row가 누적됨 → "신원이 필요한 순간"으로 발급을 미룸
