...
- **보안 영향 범위 (localStorage 탈취 시)**:
  - `nick + pw + my_decks (deck_id 리스트)` 모두 노출
  - `my_decks`의 덱들은 공격자가 **즉시 수정/삭제 가능**
  - 동일 `(nick, pw)`를 재사용한 다른 자원(다른 덱·댓글)은 공격자가 링크를 알거나 공개 발견 가능한 경우 조작 가능
  - MVP는 **XSS 방어 + enumeration 차단을 전제**로 UX 마찰 감소를 우선
