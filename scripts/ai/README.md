...
**Skill 기반 경로가 기본** — Codex/Claude 구독 내 도구로 실행하므로 Anthropic API 키가 필요 없다.

설계 배경은 `docs/feature/ai-deck-generation-pipeline.md` 참고.

## 사전 준비

주제 생성/덱 초안 생성은 Codex/Claude 스킬만 있으면 된다. 별도 Anthropic API 키는 필요 없다.
업로드 단계만 운영자 env가 필요하다.

Skill 정의:
- `.claude/skills/propose-topics/SKILL.md`
- `.claude/skills/generate-decks/SKILL.md`
- `.claude/skills/upload-decks/SKILL.md`
...
```text
/upload-decks scripts/ai/artifacts/decks/decks-<runId>.json
```

`status: "approved"`인 덱만 `/api/decks`로 업로드한다. 업로드는 AI 호출이 아니므로 Anthropic API 키가
필요 없다. `BOT_NICK`, `BOT_PW`, `BOT_SEED_TOKEN`, `API_BASE_URL`만 사용한다.
...
## 운영자 사용법 — legacy/API 스크립트 실행 (#78)

Skill 대신 코드로 주제/덱 생성을 실행하는 경로. `ai:propose-topics`, `ai:generate-decks`는 Anthropic API 키가 필요하다.
평소에는 위 Skill 경로를 사용한다.

### 환경변수

| 변수 | 용도 |
|---|---|
| `ANTHROPIC_API_KEY` | Claude API 호출 (주제/단어 추출) |
| `BOT_NICK` | 봇 닉네임 — 반드시 `bot_` prefix (예: `bot_pokemon_kr`) |
| `BOT_PW` | 봇 비밀번호 — 운영자가 보관, 시즌 업데이트 PUT에 재사용 |
| `BOT_SEED_TOKEN` | `x-bot-token` 헤더 값 — 서버의 동일 환경변수와 일치해야 bot_ nick 허용 (#77) |
| `API_BASE_URL` | 업로드 대상 (기본 `http://localhost:3000`) |

### 실행

```bash
# 1) 주제 후보 생성 → artifacts/topics/topics-<runId>.json
pnpm ai:propose-topics [category] [count]

# 2) 검수: artifact의 status를 "approved"로 수정

# 3) 단어 추출 + POST /api/decks 업로드 (--dry-run이면 artifact만)
pnpm ai:generate-decks scripts/ai/artifacts/topics/topics-<runId>.json

# 또는 이미 검수된 decks artifact만 업로드 (AI 호출 없음)
pnpm ai:upload-decks scripts/ai/artifacts/decks/decks-<runId>.json
```
