...
- script 하나만 가짐 (`latin` | `hangul` | `kana`)
...
- **gameplay 분해**: hangul script는 저장은 완성형 NFC지만 격자/키보드/비교 모두 **자모 단위**. latin/kana는 char 그대로
- **공백 불허** — 다중-단어 IP명은 분할 또는 결합 (예: "고무고무 열매" → "고무고무열매" 또는 별개 두 Word)
- **허용 문자** = `script 알파벳 + 0-9 + - + ' + .`
  - script 알파벳: latin `a-z` / hangul `가-힣` / kana `ぁ-ゖゝゞ` (히라가나)
  - 그 외 (공백, `@#$%&` 등 기호, 다른 script, control char) 모두 reject
- **정규화**: NFC + latin script만 lowercase
...
- 시작 시 게이트: `DailyRound(anon, deck, today).status ∈ {"completed", "failed"}` (라운드 종료 = 솔브 OR 시도 소진). enum은 3-state `in_progress` / `completed`(솔브) / `failed`(소진·포기)
- 시작 시 `date` 캡처
...
사용자 R이 (deck, T) thread 열람·작성 가능 ⇔
  ...
  OR (T == R.local_today
      AND DailyRound(R, deck, T).status ∈ {completed, failed}) (오늘 — 본인 라운드 종료)
